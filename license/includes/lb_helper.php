...
class LicenseBoxAPI
{
	...
	public function __construct(){
		$this->product_id = 'E7EC17E9';
		$this->api_url = 'https://licencefly.com/';
		$this->api_key = '0230756509136-shineflutr';
	...
	public function verify_license($time_based_check = false, $license = false, $client = false){
		...
		if($time_based_check && $this->verification_period > 0){
			...
			$today = date('d-m-Y');
			if(empty($_SESSION["985af7722d001b9"])){
				$_SESSION["985af7722d001b9"] = '00-00-0000';
			}
			if($type == 1){
				$type_text = '1 day';
			}elseif($type == 3){
			...
			if(strtotime($today) >= strtotime($_SESSION["985af7722d001b9"])){
				$get_data = $this->call_api(
					'POST',
